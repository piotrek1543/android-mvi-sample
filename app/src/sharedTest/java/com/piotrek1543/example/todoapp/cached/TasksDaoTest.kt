...
import com.piotrek1543.example.todoapp.MainCoroutineRule
...
class TasksDaoTest {
    ...
    private var entityMapper = TaskEntityMapper()
    ...
    fun insertTaskAndGetById() = runBlockingTest {
        ...
        val cachedTask = entityMapper.mapToCached(task)
        database.taskDao().insertTask(cachedTask)

        // WHEN - Get the task by id from the database
        val loaded = database.taskDao().getTaskById(task.id)

        // THEN - The loaded data contains the expected values
        assertThat(loaded as CachedTask, notNullValue())
    ...
    fun insertTaskReplacesOnConflict() = runBlockingTest {
        ...
        val cachedNewTask = entityMapper.mapToCached(newTask)
        database.taskDao().insertTask(cachedNewTask)
    ...
    fun updateTaskAndGetById() = runBlockingTest {
        ...
        val cachedOriginalTask = entityMapper.mapToCached(originalTask)
        database.taskDao().insertTask(cachedOriginalTask)

        // When the task is updated
        val updatedTask = Task("new title", "new description", true, originalTask.id)
        val cachedTask = entityMapper.mapToCached(updatedTask)
        database.taskDao().updateTask(cachedTask)

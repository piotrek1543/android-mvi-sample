...
import com.piotrek1543.example.todoapp.util.monitorFragment
...
class StatisticsFragmentTest {
    ...
    fun tasks_showsNonEmptyMessage() {
        ...
        onView(withId(R.id.text_stats_active)).check(matches(isDisplayed()))
        onView(withId(R.id.text_stats_active)).check(matches(withText(expectedActiveTaskText)))
        onView(withId(R.id.text_stats_completed)).check(matches(isDisplayed()))
        onView(withId(R.id.text_stats_completed)).check(matches(withText(expectedCompletedTaskText)))
    }
}
